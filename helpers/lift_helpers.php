...
class LIFT_Helpers extends WPBakeryShortCode {
	...
	public function applyHeadingCustomPost($title = null,$tag = null,$url = false,$attr = null) {
		$styles = '';
		if($url) {
			$styles = '<'.$tag.' class="lift-title"><a href="'.$url.'" class="lift-title-link"'.($attr ? ' '.$attr : '').'>' .  $title . '</a></'.$tag.'>';
		} else {
			$styles = '<'.$tag.' class="lift-title">' .  $title . '</'.$tag.'>';
		}
		return $styles;
	}
	public function applyHeadingMagic($title = null,$tag = null,$wrapTag = null,$icon = null) {
		$styles = '';
		if($wrapTag) {
			$styles = '<'.$tag.' class="lift-title">'.$icon.'<'.$wrapTag.' class="lift-title-wrap">' .  $title . '</'.$wrapTag.'></'.$tag.'>';
		} else {
			$styles = '<'.$tag.' class="lift-title">' .  $title . '</'.$tag.'>';
		}
		return $styles;
	...
	public function extractLinkCustomPost($link, $title,$enabletitle=true) {
		$returnURL = '';
		if($enabletitle) {
			$returnURL = '<a class="lift-title-link" href="'.$link.'">'. $title .'</a>';
		} else {
			$returnURL = '<a class="lift-title-link hold-click" href="'.$link.'"></a>';
		}
		return $returnURL;
	}
