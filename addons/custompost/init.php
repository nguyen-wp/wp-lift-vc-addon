...
if (!class_exists('liftVC_Addons_CustomPost')) {
	...
	class liftVC_Addons_CustomPost extends LIFT_Helpers
	{
		...
		public $pNicename = 'LIFT Magic Posts';
			...
			return array(
				...
				'params'      => array(
					...
					array(
						...
						'value' => array(
							esc_html__( 'Default', 'js_composer' ) => 'default',
							esc_html__( 'Display 1', 'js_composer' ) => 'lift-display-1',
							esc_html__( 'Display 2', 'js_composer' ) => 'lift-display-2',
							esc_html__( 'Display 3', 'js_composer' ) => 'lift-display-3',
							esc_html__( 'Display 4', 'js_composer' ) => 'lift-display-4',
							esc_html__( 'Display 5', 'js_composer' ) => 'lift-display-5',
							esc_html__( 'Display 6', 'js_composer' ) => 'lift-display-6',
							esc_html__( 'Display 7 (fs-1)', 'js_composer' ) => 'lift-font-1',
							esc_html__( 'Display 8 (fs-2)', 'js_composer' ) => 'lift-font-2',
							esc_html__( 'Display 9 (fs-3)', 'js_composer' ) => 'lift-font-3',
							esc_html__( 'Display 10 (fs-4)', 'js_composer' ) => 'lift-font-4',
							esc_html__( 'Display 11 (fs-5)', 'js_composer' ) => 'lift-font-5',
							esc_html__( 'Display 12 (fs-6)', 'js_composer' ) => 'lift-font-6',
						),
		...
		public function output($atts, $content = null)
		{
			...
			$full_height = $atts["full_height"] ? ' lift-full-height': '';
			...
			$links_layout = $atts["links_layout"] ? ' '. $atts["links_layout"] : ' lift-text-left';
			$column_layout = $atts["column_layout"] ? ' lift-'. $atts["column_layout"] : ' lift-1-column';
			$masonry_layout = $masonry ? ' lift-masonry-layout' : '';
			...
			$css_build .= $box_padding ? ''. $cssname.' .lift-layout-posts .lift-box{padding:'.$box_padding.'!important;}' : null;

			if($masonry) {
				$css_build .= $box_margin ? ''. $cssname.' .lift-layout-posts {-webkit-column-gap: '.$box_margin.'!important; -moz-column-gap: '.$box_margin.'!important; column-gap: '.$box_margin.'!important;}' : null;
				$css_build .= $box_margin ? ''. $cssname.' .lift-layout-posts .lift-ctn{margin-bottom: '.$box_margin.'!important;}' : null;
				// BOX
				$css_build .= $box_radius ? ''. $cssname.' .lift-layout-posts .lift-ctn{border-radius:'.$box_radius.'!important;}' : null;
				$css_build .= $box_radius_hover ? ''. $cssname.' .lift-layout-posts .lift-ctn:hover{border-radius:'.$box_radius_hover.'!important;}' : null;
				$css_build .= $box_border_size ? ''. $cssname.' .lift-layout-posts .lift-ctn{border-style:solid!important;border-width:'.$box_border_size.'!important;}' : null;
				$css_build .= $box_transition ? ''. $cssname.' .lift-layout-posts .lift-ctn{transition:all ease-in-out '.$box_transition.'!important;}' : null;
				$css_build .= $box_border ? ''. $cssname.' .lift-layout-posts .lift-ctn {border-color:'.$this->liftHexToRGB($box_border).'!important;}' : null;
				$css_build .= $box_border_hover ? ''. $cssname.' .lift-layout-posts .lift-ctn:hover {border-color:'.$this->liftHexToRGB($box_border_hover).'!important;}' : null;
				$css_build .= $box_background ? ''. $cssname.' .lift-layout-posts .lift-ctn{background-color:'.$this->liftHexToRGB($box_background).'!important;}' : null;
				$css_build .= $box_background_hover ? ''. $cssname.' .lift-layout-posts .lift-ctn:hover{background-color:'.$this->liftHexToRGB($box_background_hover).'!important;}' : null;
			} else {
				$css_build .= $box_margin ? ''. $cssname.' .lift-layout-posts {margin-left:-'.$box_margin.'!important;margin-right:-'.$box_margin.'!important;}' : null;
				$css_build .= $box_margin ? ''. $cssname.' .lift-layout-posts .lift-ctn{padding-left:'.$box_margin.'!important;padding-right:'.$box_margin.'!important;margin-bottom: '.$box_margin.'!important;}' : null;
				// BOX
				$css_build .= $box_radius ? ''. $cssname.' .lift-layout-posts .lift-ctn .lift-box{border-radius:'.$box_radius.'!important;}' : null;
				$css_build .= $box_radius_hover ? ''. $cssname.' .lift-layout-posts .lift-ctn .lift-box:hover{border-radius:'.$box_radius_hover.'!important;}' : null;
				$css_build .= $box_border_size ? ''. $cssname.' .lift-layout-posts .lift-ctn .lift-box{border-style:solid!important;border-width:'.$box_border_size.'!important;}' : null;
				$css_build .= $box_transition ? ''. $cssname.' .lift-layout-posts .lift-ctn .lift-box{transition:all ease-in-out '.$box_transition.'!important;}' : null;
				$css_build .= $box_border ? ''. $cssname.' .lift-layout-posts .lift-ctn .lift-box {border-color:'.$this->liftHexToRGB($box_border).'!important;}' : null;
				$css_build .= $box_border_hover ? ''. $cssname.' .lift-layout-posts .lift-ctn .lift-box:hover {border-color:'.$this->liftHexToRGB($box_border_hover).'!important;}' : null;
				$css_build .= $box_background ? ''. $cssname.' .lift-layout-posts .lift-ctn .lift-box{background-color:'.$this->liftHexToRGB($box_background).'!important;}' : null;
				$css_build .= $box_background_hover ? ''. $cssname.' .lift-layout-posts .lift-ctn .lift-box:hover{background-color:'.$this->liftHexToRGB($box_background_hover).'!important;}' : null;
			}


			// BUTTON 
			$css_build .= $btn_color ? ''. $cssname.' .lift-ctn .lift-button >*{color:'.$this->liftHexToRGB($btn_color).'!important}' : null;
			$css_build .= $btn_bg ? ''. $cssname.' .lift-ctn .lift-button >*{background-color:'.$this->liftHexToRGB($btn_bg,$btn_transparent).'!important}' : null;
			$css_build .= $btn_border_color ? ''. $cssname.' .lift-ctn .lift-button >*{border-color:'.$this->liftHexToRGB($btn_border_color).'!important}' : null;
			$css_build .= $btn_border ? ''. $cssname.' .lift-ctn .lift-button >*{border-style:solid!important;border-width:'.$btn_border.'!important;}' : null;
			if($btn_border_style) {
				if($btn_border_style === 'top') {
					$css_build .= ''. $cssname.' .lift-ctn .lift-button >*{border-left-width:0!important;border-right-width:0!important;border-bottom-width:0!important}';
				} else if($btn_border_style === 'bottom') {
					$css_build .= ''. $cssname.' .lift-ctn .lift-button >*{border-left-width:0!important;border-right-width:0!important;border-top-width:0!important}';
				} else if($btn_border_style === 'left') {
					$css_build .= ''. $cssname.' .lift-ctn .lift-button >*{border-top-width:0!important;border-right-width:0!important;border-bottom-width:0!important}';
				} else if($btn_border_style === 'right') {
					$css_build .= ''. $cssname.' .lift-ctn .lift-button >*{border-top-width:0!important;border-left-width:0!important;border-bottom-width:0!important}';
				}
			}
			$css_build .= $btn_border_radius ? ''. $cssname.' .lift-ctn .lift-button >*{border-radius:'.$btn_border_radius.'!important;}' : null;
			$css_build .= $btn_padding ? ''. $cssname.' .lift-ctn .lift-button >*{padding:'.$btn_padding.'!important;}' : null;
			$css_build .= $btn_margin ? ''. $cssname.' .lift-ctn .lift-button >*{margin:'.$btn_margin.'!important;}' : null;
			$css_build .= ($btn_decoration && $btn_decoration === 'yes') ? ''. $cssname.' .lift-ctn .lift-button >*{text-decoration: none!important;}' : null;

			if($add_box_button){
				$css_build .= $btn_hover_color ? ''. $cssname.' .lift-ctn:hover .lift-button >*{color:'.$this->liftHexToRGB($btn_hover_color).'!important}' : null;
				$css_build .= $btn_hover_color ? ''. $cssname.' .lift-ctn:focus .lift-button >*{color:'.$this->liftHexToRGB($btn_hover_color).'!important}' : null;
				$css_build .= $btn_hover_color ? ''. $cssname.' .lift-ctn:active .lift-button >*{color:'.$this->liftHexToRGB($btn_hover_color).'!important}' : null;
				$css_build .= $btn_hover_bg ? ''. $cssname.' .lift-ctn:hover .lift-button >*{background-color:'.$this->liftHexToRGB($btn_hover_bg,$btn_transparent_hover).'!important}' : null;
				$css_build .= $btn_hover_bg ? ''. $cssname.' .lift-ctn:focus .lift-button >*{background-color:'.$this->liftHexToRGB($btn_hover_bg,$btn_transparent_hover).'!important}' : null;
				$css_build .= $btn_hover_bg ? ''. $cssname.' .lift-ctn:active .lift-button >*{background-color:'.$this->liftHexToRGB($btn_hover_bg,$btn_transparent_hover).'!important}' : null;
				$css_build .= $btn_border_hover ? ''. $cssname.' .lift-ctn:hover .lift-button >*{border-color:'.$this->liftHexToRGB($btn_border_hover).'!important}' : null;
				$css_build .= $btn_border_hover ? ''. $cssname.' .lift-ctn:focus .lift-button >*{border-color:'.$this->liftHexToRGB($btn_border_hover).'!important}' : null;
				$css_build .= $btn_border_hover ? ''. $cssname.' .lift-ctn:active .lift-button >*{border-color:'.$this->liftHexToRGB($btn_border_hover).'!important}' : null;
			} else {
				$css_build .= $btn_hover_color ? ''. $cssname.' .lift-ctn .lift-button >*:hover{color:'.$this->liftHexToRGB($btn_hover_color).'!important}' : null;
				$css_build .= $btn_hover_color ? ''. $cssname.' .lift-ctn .lift-button >*:focus{color:'.$this->liftHexToRGB($btn_hover_color).'!important}' : null;
				$css_build .= $btn_hover_color ? ''. $cssname.' .lift-ctn .lift-button >*:active{color:'.$this->liftHexToRGB($btn_hover_color).'!important}' : null;
				$css_build .= $btn_hover_bg ? ''. $cssname.' .lift-ctn .lift-button >*:hover{background-color:'.$this->liftHexToRGB($btn_hover_bg,$btn_transparent_hover).'!important}' : null;
				$css_build .= $btn_hover_bg ? ''. $cssname.' .lift-ctn .lift-button >*:focus{background-color:'.$this->liftHexToRGB($btn_hover_bg,$btn_transparent_hover).'!important}' : null;
				$css_build .= $btn_hover_bg ? ''. $cssname.' .lift-ctn .lift-button >*:active{background-color:'.$this->liftHexToRGB($btn_hover_bg,$btn_transparent_hover).'!important}' : null;
				$css_build .= $btn_border_hover ? ''. $cssname.' .lift-ctn .lift-button >*:hover{border-color:'.$this->liftHexToRGB($btn_border_hover).'!important}' : null;
				$css_build .= $btn_border_hover ? ''. $cssname.' .lift-ctn .lift-button >*:focus{border-color:'.$this->liftHexToRGB($btn_border_hover).'!important}' : null;
				$css_build .= $btn_border_hover ? ''. $cssname.' .lift-ctn .lift-button >*:active{border-color:'.$this->liftHexToRGB($btn_border_hover).'!important}' : null;
			}
			$css_build .= $btn_line_height ? ''. $cssname.' .lift-ctn .lift-button .lift-title-link{line-height:'.$btn_line_height.'!important;}' : null;
			// PAGED 
			$css_build .= $page_color ? ''. $cssname.' .lift-paging >*{color:'.$this->liftHexToRGB($page_color).'!important}' : null;
			$css_build .= $page_bg ? ''. $cssname.' .lift-paging >*{background-color:'.$this->liftHexToRGB($page_bg,$page_transparent).'!important}' : null;
			$css_build .= $page_border_color ? ''. $cssname.' .lift-paging >*{border-color:'.$this->liftHexToRGB($page_border_color).'!important}' : null;
			$css_build .= $page_border ? ''. $cssname.' .lift-paging >*{border-style:solid!important;border-width:'.$page_border.'!important;}' : null;
			if($page_border_style) {
				if($page_border_style === 'top') {
					$css_build .= ''. $cssname.' .lift-paging >*{border-left-width:0!important;border-right-width:0!important;border-bottom-width:0!important}';
				} else if($page_border_style === 'bottom') {
					$css_build .= ''. $cssname.' .lift-paging >*{border-left-width:0!important;border-right-width:0!important;border-top-width:0!important}';
				} else if($page_border_style === 'left') {
					$css_build .= ''. $cssname.' .lift-paging >*{border-top-width:0!important;border-right-width:0!important;border-bottom-width:0!important}';
				} else if($page_border_style === 'right') {
					$css_build .= ''. $cssname.' .lift-paging >*{border-top-width:0!important;border-left-width:0!important;border-bottom-width:0!important}';
				}
			}
			$css_build .= $page_border_radius ? ''. $cssname.' .lift-paging >*{border-radius:'.$page_border_radius.'!important;}' : null;
			$css_build .= $page_padding ? ''. $cssname.' .lift-paging >*{padding:'.$page_padding.'!important;}' : null;
			$css_build .= $page_margin ? ''. $cssname.' .lift-paging >*{margin:'.$page_margin.'!important;}' : null;
			$css_build .= ($page_decoration && $page_decoration === 'yes') ? ''. $cssname.' .lift-paging >*{text-decoration: none!important;}' : null;
            $css_build .= $page_hover_color ? ''. $cssname.' .lift-paging >*:hover{color:'.$this->liftHexToRGB($page_hover_color).'!important}' : null;
            $css_build .= $page_hover_color ? ''. $cssname.' .lift-paging >*:focus{color:'.$this->liftHexToRGB($page_hover_color).'!important}' : null;
            $css_build .= $page_hover_color ? ''. $cssname.' .lift-paging >*:active{color:'.$this->liftHexToRGB($page_hover_color).'!important}' : null;
            $css_build .= $page_hover_bg ? ''. $cssname.' .lift-paging >*:hover{background-color:'.$this->liftHexToRGB($page_hover_bg,$page_transparent_hover).'!important}' : null;
            $css_build .= $page_hover_bg ? ''. $cssname.' .lift-paging >*:focus{background-color:'.$this->liftHexToRGB($page_hover_bg,$page_transparent_hover).'!important}' : null;
            $css_build .= $page_hover_bg ? ''. $cssname.' .lift-paging >*:active{background-color:'.$this->liftHexToRGB($page_hover_bg,$page_transparent_hover).'!important}' : null;
            $css_build .= $page_border_hover ? ''. $cssname.' .lift-paging >*:hover{border-color:'.$this->liftHexToRGB($page_border_hover).'!important}' : null;
            $css_build .= $page_border_hover ? ''. $cssname.' .lift-paging >*:focus{border-color:'.$this->liftHexToRGB($page_border_hover).'!important}' : null;
            $css_build .= $page_border_hover ? ''. $cssname.' .lift-paging >*:active{border-color:'.$this->liftHexToRGB($page_border_hover).'!important}' : null;
			$css_build .= $page_line_height ? ''. $cssname.' .lift-paging {line-height:'.$page_line_height.'!important;}' : null;
			...
			$output .= '<section'. $block_id .' class="lift-elements lift-' . $this->name . $css_class. $theme.$classname.$column_layout.$masonry_layout.$cssaddclass.$css_animation.$full_height.'"' . str_replace('``', '', $attribute) . '>';
			...
			$output .= '<div class="lift-layout-posts clearfix">';
			...
			$output .= '<nav class="lift-paging'.$links_layout.''.$page_font_size.'">'.$pageding.'</nav>';
