...
if (!class_exists('liftVC_Addons_Heading')) {

	class liftVC_Addons_Heading extends LIFT_Helpers
	{

		public $name = 'heading';
		public $pNicename = 'LIFT Heading';
		...
		public function output($atts, $content = null)
		{
			...
			$css_build .= $icon_padding ? ''. $cssname.' .lift-icon {padding:'.$icon_padding.'!important;}' : null;
			$css_build .= $icon_margin ? ''. $cssname.' .lift-icon {margin:'.$icon_margin.'!important;}' : null;
			...
			$css_build .= $text_color ? ''. $cssname.' .lift-title {color:'.$this->liftHexToRGB($text_color).'!important}' : null;
			$css_build .= $text_secondary_color ? ''. $cssname.' .lift-title ins {color:'.$this->liftHexToRGB($text_secondary_color).'!important}' : null;
			$css_build .= $text_color_hover ? ''. $cssname.':hover .lift-title {color:'.$this->liftHexToRGB($text_color_hover).'!important}' : null;
			$css_build .= $text_color_hover ? ''. $cssname.':focus .lift-title {color:'.$this->liftHexToRGB($text_color_hover).'!important}' : null;
			$css_build .= $text_color_hover ? ''. $cssname.':active .lift-title {color:'.$this->liftHexToRGB($text_color_hover).'!important}' : null;
			$css_build .= $text_secondary_color_hover ? ''. $cssname.':hover .lift-title ins{color:'.$this->liftHexToRGB($text_secondary_color_hover).'!important}' : null;
			$css_build .= $text_secondary_color_hover ? ''. $cssname.':focus .lift-title ins{color:'.$this->liftHexToRGB($text_secondary_color_hover).'!important}' : null;
			$css_build .= $text_secondary_color_hover ? ''. $cssname.':active .lift-title ins{color:'.$this->liftHexToRGB($text_secondary_color_hover).'!important}' : null;
			$css_build .= $icon_size ? $cssname.' .lift-icon{'.$applyIconStyle.'!important}' : null;
			// LINER
			$css_build .= $text_background ? ''. $cssname.' .lift-title > * {background-color:'.$this->liftHexToRGB($text_background).'!important}' : null;
			$css_build .= $heading_margin ? ''. $cssname.' .lift-title > * {margin:'.$heading_margin.'!important}' : null;
			$css_build .= $heading_padding ? ''. $cssname.' .lift-title > * {padding:'.$heading_padding.'!important}' : null;
			// STYLE1
			$css_build .= $line_height ? ''. $cssname.'.lift-theme-liner:after {height:'.$line_height.'!important}' : null;
			$css_build .= $line_top ? ''. $cssname.'.lift-theme-liner:after {bottom:'.$line_top.'!important}' : null;
			$css_build .= $line_mobile_width ? ''. $cssname.'.lift-theme-liner:after {width:'.$line_mobile_width.'!important}' : null;
			if($line_tablet_width) {
				$css_build .= $line_tablet_width ? '@media only screen and (min-width: 767px) {'. $cssname.'.lift-theme-liner:after {width:'.$line_tablet_width.'!important}}' : null;
			}
			if($line_desktop_width) {
				$css_build .= $line_desktop_width ? '@media only screen and (min-width: 999px) {'. $cssname.'.lift-theme-liner:after {width:'.$line_desktop_width.'!important}}' : null;
			}
			$css_build .= $line_color ? ''. $cssname.'.lift-theme-liner:after {background-color:'.$this->liftHexToRGB($line_color).'!important}' : null;
			$css_build .= $line_color_hover ? ''. $cssname.'.lift-theme-liner:hover:after {background-color:'.$this->liftHexToRGB($line_color_hover).'!important}' : null;
			// STYLE2
			$css_build .= $line_height ? ''. $cssname.'.lift-theme-top:after {height:'.$line_height.'!important}' : null;
			$css_build .= $line_top ? ''. $cssname.'.lift-theme-top:after {top:'.$line_top.'!important}' : null;
			$css_build .= $line_mobile_width ? ''. $cssname.'.lift-theme-top:after {width:'.$line_mobile_width.'!important}' : null;
			if($line_tablet_width) {
				$css_build .= $line_tablet_width ? '@media only screen and (min-width: 767px) {'. $cssname.'.lift-theme-top:after {width:'.$line_tablet_width.'!important}}' : null;
			}
			if($line_desktop_width) {
				$css_build .= $line_desktop_width ? '@media only screen and (min-width: 999px) {'. $cssname.'.lift-theme-top:after {width:'.$line_desktop_width.'!important}}' : null;
			}
			$css_build .= $line_color ? ''. $cssname.'.lift-theme-top:after {background-color:'.$line_color.'!important}' : null;
			$css_build .= $line_color_hover ? ''. $cssname.'.lift-theme-top:hover:after {background-color:'.$this->liftHexToRGB($line_color_hover).'!important}' : null;
			// STYLE3
			$css_build .= $line_height ? ''. $cssname.'.lift-theme-middle .lift-title span:after {height:'.$line_height.'!important}' : null;
			$css_build .= $line_top ? ''. $cssname.'.lift-theme-middle .lift-title span:after {bottom:'.$line_top.'!important}' : null;
			$css_build .= $line_mobile_width ? ''. $cssname.'.lift-theme-middle .lift-title span:after {width:'.$line_mobile_width.'!important}' : null;
			if($line_tablet_width) {
				$css_build .= $line_tablet_width ? '@media only screen and (min-width: 767px) {'. $cssname.'.lift-theme-middle .lift-title span:after {width:'.$line_tablet_width.'!important}}' : null;
			}
			if($line_desktop_width) {
				$css_build .= $line_desktop_width ? '@media only screen and (min-width: 999px) {'. $cssname.'.lift-theme-middle .lift-title span:after {width:'.$line_desktop_width.'!important}}' : null;
			}
			$css_build .= $line_color ? ''. $cssname.'.lift-theme-middle .lift-title span:after {background-color:'.$this->liftHexToRGB($line_color).'!important}' : null;
			$css_build .= $line_color_hover ? ''. $cssname.'.lift-theme-middle:hover .lift-title span:after {background-color:'.$this->liftHexToRGB($line_color_hover).'!important}' : null;

			// FrontEnd
			$output = ($css || $css_build) ? '<style>' . $css .$css_build. '</style>' : '';
			$output .= '<section'. $block_id .' class="lift-elements lift-' . $this->name . $css_class.$theme. $classname.$cssaddclass.$align.$css_animation.$line_align.'"' . str_replace('``', '', $attribute) . $delay.'>';
